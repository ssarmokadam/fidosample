...
public class FidoServerAssertionResultEndpointFilter extends AbstractAuthenticationProcessingFilter {
  ...
  private final TypeReference<ServerPublicKeyCredential<ServerAuthenticatorAssertionResponse>> credentialTypeRef = new TypeReference<ServerPublicKeyCredential<ServerAuthenticatorAssertionResponse>>() {
  };
  ...
  public FidoServerAssertionResultEndpointFilter(ObjectConverter objectConverter,
      ServerPropertyProvider serverPropertyProvider, RequestMatcher requiresAuthenticationRequestMatcher) {
    ...
    setAuthenticationSuccessHandler(new FidoServerAssertionResultEndpointSuccessHandler(objectConverter));
    setAuthenticationFailureHandler(new FidoServerAssertionResultEndpointFailureHandler(objectConverter));
  ...
  public FidoServerAssertionResultEndpointFilter(ObjectConverter objectConverter,
      ServerPropertyProvider serverPropertyProvider, String defaultFilterProcessesUrl) {

    this(objectConverter, serverPropertyProvider,
        new AntPathRequestMatcher(defaultFilterProcessesUrl, HttpMethod.POST.name()));
  }
  ...
  private void checkConfig() {
    ...
    Assert.notNull(this.serverPropertyProvider, "serverPropertyProvider must not be null");
  }
  ...
  public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ServerPublicKeyCredential<ServerAuthenticatorAssertionResponse> credential = this.jsonConverter
          .readValue(inputStream, this.credentialTypeRef);
      ...
      CollectedClientData collectedClientData = this.collectedClientDataConverter
          .convert(assertionResponse.getClientDataJSON());
      UserVerificationRequirement userVerificationRequirement = this.serverEndpointFilterUtil
          .decodeUserVerification(collectedClientData.getChallenge());
      ...
      WebAuthnAuthenticationRequest webAuthnAuthenticationRequest = new WebAuthnAuthenticationRequest(
          ...
          assertionResponse.getClientDataJSON() == null ? null
              : Base64UrlUtil.decode(assertionResponse.getClientDataJSON()),
          assertionResponse.getAuthenticatorData() == null ? null
      ...
      WebAuthnAuthenticationParameters webAuthnAuthenticationParameters = new WebAuthnAuthenticationParameters(
          serverProperty, userVerificationRequirement == UserVerificationRequirement.REQUIRED, false);

      WebAuthnAssertionAuthenticationToken webAuthnAssertionAuthenticationToken = new WebAuthnAssertionAuthenticationToken(
          webAuthnAuthenticationRequest, webAuthnAuthenticationParameters, Collections.emptyList());
